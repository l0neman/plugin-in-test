package io.l0neman.plugintarget.util;
...
public class TALogger {
  ...
  private static final String MAIN_TAG = "TA";
  ...
  private static int getStackDepth(StackTraceElement[] elements) {
    for (int i = STACK_DEPTH; i < elements.length; i++) {
      ...
      if (!element.getClassName().split("\\$")[0].equals(TALogger.class.getName())) {
        return i;
      }
    }
